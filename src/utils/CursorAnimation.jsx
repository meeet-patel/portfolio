...
const VARIANT_RING = {
  default:
    "border-2 border-accent/60 bg-accent/10 backdrop-blur-[1px]",
};
...
export default function CustomCursor({ variant = "default" }) {
  ...
  useEffect(() => {
    ...
    const handleMouseOverText = () => setCursorSize(56);
  ...
  return (
    <motion.div
      className={`pointer-events-none fixed z-[60] hidden rounded-full md:block ${VARIANT_RING[variant] ?? VARIANT_RING.default}`}
